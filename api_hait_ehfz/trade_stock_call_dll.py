...
def OnRecvData(service_id, funcid, pdata, ndata, pRspInfo, nrequestid):
    ...
    if funcid == TRADE_FUNCID_TYPE.JG_FUNCID_STOCK_Login.value:
        ...
        if ANSINFO.contents.nResultType == JG_TDC_ANSRESULT_Success:
            ...
            while i < ANSINFO.contents.nFieldItem:
                ...
                print("[应答 Link %d]下单应答成功  代码：%s, 市场：%d, 买卖方向：%d, 价格：%d,数量：%d, 合同号：%s"
                      % (service_id, Ans.contents.szStockCode.decode("gb2312", errors='ignore'),
        ...
        if ANSINFO.contents.nResultType == JG_TDC_ANSRESULT_Success:
            ...
            with open(fpath_cacct_fund, 'w') as f:
                ...
                while i < ANSINFO.contents.nFieldItem:
                    ...
                    print(
                        "[应答 Link %d]资金查询成功 币种：%s, 可用余额：%.2f, 可取余额：%.2f, 冻结金额：%.2f, 证券市值：%.2f, 资金余额：%.2f, 资产总值：%.2f, 总盈亏：%.2f "
                        % (service_id, Ans.contents.cMoneyType.decode("gb2312", errors='ignore'),
                           ...
                           Ans.contents.dAssetBalance, Ans.contents.dInCome))
        ...
        if ANSINFO.contents.nResultType == JG_TDC_ANSRESULT_Success:
            ...
            print(
                "[应答 Link %d]撤单成功, 合同号：%s" % (g_serviceid, Ans.contents.szEntrustNo.decode("gb2312", errors='ignore')))
        else:
        ...
        if ANSINFO.contents.nResultType == JG_TDC_ANSRESULT_Success:
            ...
            print("[应答 Link %d]最大交易数量查询失败， Error：%s" % (
            service_id, ANSINFO.contents.szErrorInfo.decode("gb2312", errors='ignore')))
    else:
        print("未知的数据回调...")
...
def QryBusiness(g_serviceid):
    ...
    req.nQueryDirection = int(bytes(JG_TDC_QUERYDIRECTION_Inverted, encoding="gb2312"))
    temp = cast(pointer(req), c_char_p)
    if (0 == API_TradeSend(g_serviceid, TRADE_FUNCID_TYPE.JG_FUNCID_STOCK_QryBusByPos.value, temp, 1, 0)):
        print("查询成交成功")
    else:
        print("查询成交失败")
